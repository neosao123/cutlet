...
@push('styles')
    ...
    <link href="{{ asset('assets/init_site/restaurant/pendingorder/index.css') }}" rel="stylesheet">
        ...
        <div class="card">
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-sm-2">
                        <a href="{{ url('restaurantPendingOrder/list') }}" class="btn btn-outline-primary btn-sm"> Back </a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <form action="{{ url('restaurantPendingOrder/confirm')}}" method="post" novalidate>
				@csrf
				@if($query)
					...
					<div class="form-row">
						...
						<hr>
    						...
    						@if(in_array($row->orderStatus ,['PND','PLC']))
							<div class="text-xs-right">
								 <button type="submit" id="submit" class="btn btn-success" onclick="page_isPostBack=true;">Confirm</button>
								 <button type="button" id="discard" class="btn btn-danger" onclick="page_isPostBack=true;">Reject</button>
							</div>
					        @endif 
						@endforeach
					@endif
...
@push('scripts')
  ...
  <script type="text/javascript" src="{{ asset('assets/theme/dist/js/pages/datatable/datatable-basic.init.js') }}"></script>
	...
	<script type="text/javascript" src="{{ asset('assets/init_site/restaurant/pendingorder/view.js') }}"></script>
