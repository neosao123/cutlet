...
function ProcessExcel(data) {
	...
	for (var i = 0; i < excelRows.length; i++) {
        ...
        var salePrice = cell1.innerHTML = excelRows[i][Object.keys(excelRows[i])[1]];

        cell2 = row.insertCell(-1);
        var packagingCharges = cell2.innerHTML = excelRows[i][Object.keys(excelRows[i])[2]];
		
        cell3 = row.insertCell(-1);
        var maxOrderQty = cell3.innerHTML = excelRows[i][Object.keys(excelRows[i])[3]];

        cell4 = row.insertCell(-1);
        var cuisineType = cell4.innerHTML = excelRows[i][Object.keys(excelRows[i])[4]];

		cell5 = row.insertCell(-1);
        var itemImage = cell5.innerHTML = 'Image';

        cell6 = row.insertCell(-1);
        var itemDescription = cell6.innerHTML = excelRows[i][Object.keys(excelRows[i])[5]];

        cell7 = row.insertCell(-1);
        var menuCategory = cell7.innerHTML = excelRows[i][Object.keys(excelRows[i])[6]];                 
         
		cell8 = row.insertCell(-1);
        var menuSubcategory = cell8.innerHTML = excelRows[i][Object.keys(excelRows[i])[7]];
		
		cell9 = row.insertCell(-1);
        var itemStatus = cell9.innerHTML = excelRows[i][Object.keys(excelRows[i])[8]];
	...
	$('#modalForm').on('submit', function(e){ 
		...
		$.ajax({ 
			...
			type:'POST',
			url: baseUrl + '/restaurantItems/uploadData',
			data: formData,
			contentType: false,
			cache: false,
			processData:false,	
	...
	function validateExcelRows(){
	   ...
	   $.ajax({
            url: baseUrl + "/restaurantItems/validateExcelFile",
			async:false,
			dataType:'JSON',
            type: 'POST',
            data: {
