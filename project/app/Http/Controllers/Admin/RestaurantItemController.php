...
class RestaurantItemController extends Controller
{
    ...
    public function index()
    {
		...
		$data['restaurantitemmaster']=DB::table('restaurantitemmaster')->distinct()->select('itemName')->where('isDelete','!=',1)->get();
		//$data['restaurantitemmaster'] = $this->model->selectActiveDataFromTable('restaurantitemmaster');
        ...
        return view('admin.restaurantitem.list',$data);
    }

    public function getItemList(Request $req)
    {
        ...
        $condition = array('restaurantitemmaster.restaurantCode' => array('=', $restname),'restaurantitemmaster.itemName' => array('=', $restitem),'restaurantitemmaster.isAdminApproved' => array('=', $approvedstatus),'restaurantitemmaster.menuCategoryCode'=>array('=',$menucategory));
        ...
        $like = array('restaurantitemmaster.itemName' => $search,"menucategory.menuCategoryName"=>$search,"restaurant.entityName"=>$search);
        $limit = $req->length;
        $offset = $req->start;
        $extraCondition = " (`restaurantitemmaster`.`isDelete`=0  OR `restaurantitemmaster`.`isDelete` IS NULL)";
		//DB::enableQueryLog();
            ...
            foreach ($result as $row) {
                ...
                $actions = '<div class="btn-group">
					...
					<div class="dropdown-menu animated slideInUp" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 35px, 0px);">
						<a class="dropdown-item" href="' . url("restaurantItem/view/" . $row->code) . '"><i class="ti-eye"></i> Open</a>
						<a class="dropdown-item" href="' . url("restaurantItem/edit/" . $row->code) . '"><i class="ti-pencil-alt"></i> Edit</a>
						<a class="dropdown-item" href="' . url("restaurantItem/customizeaddon/" . $row->code) . '"><i class="ti-pencil"></i> Add Ons</a>
    ...
    public function add()
    {
        ...
        return view('admin.restaurantitem.add',$data);
    }
	...
	public function customizeaddon($code)
    {
		...
		//DB::enableQueryLog();
		$condition = array("customizedcategory.isDelete" => array('!=',1),"customizedcategory.restaurantItemCode" => array('=', $code));
		...
		$condition2 = array("customizedcategory.isDelete" => array('!=', 1),"customizedcategory.restaurantItemCode" => array('=', $code));
        ...
        return view('admin.restaurantitem.addon', $data);
    }
    ...
    public function store(Request $r)
    {
        ...
        $rules = array(
			'itemName' => 'required|regex:/^[a-zA-Z\s]+$/|min:1|max:80',
            'restaurantCode' => 'required',
            'cuisineType' => 'required',
            'menuCategoryCode' => 'required',
            'salePrice' => 'required|regex:/^[0-9\s]+$/',
        );
        $messages = array(
            'itemName.required' => 'Item Name is required',
            'itemName.regex' => 'Invalid characters like number, special characters are not allowed',
            'itemName.min' => 'Minimum of 1 characters are required.',
            'itemName.max' => 'Max characters exceeded.',
			...
			'salePrice.required' => 'Only numbers are  required'
        );
        $data = [
			'itemName' => trim($r->itemName),
			'itemDescription' => trim($r->itemDescription),
			'restaurantCode' => trim($r->restaurantCode),
        ...
        if ($currentId) {
			if ($r->hasFile('itemImage')) {
				$path = 'uploads/restaurant/restaurantitemimage/'.$r->restaurantCode;
				if (! File::exists($path)) {
				...
				$image_update = $this->model->doEdit($itemImage, 'restaurantitemmaster', $currentId);
			}
			//activity log start
			$data=$currentdate->toDateTimeString().	"	"	.$ip.	"	"	.Auth::guard('admin')->user()->code.	"	"."Restaurant Item ".$currentId." is added";
			$this->model->activity_log($data); 
			//activity log end
						
            return redirect('restaurantItem/list')->with('success', 'Restaurant item added successfully');
        }
    ...
    public function edit($code)
    {
       ...
       $data['restaurant'] = $this->model->selectActiveDataFromTable('restaurant');
		...
		if($menuCategoryCode!='' && $menuCategoryCode!=null){
			$data['menuSubcategory'] = $this->model->selectQuery(array('menusubcategory.code','menusubcategory.menuSubCategoryName'),'menusubcategory',array(),array('menusubcategory.isActive' => array('=', 1),'menusubcategory.menuCategoryCode' => array('=', $menuCategoryCode)));
		}
        return view('admin.restaurantitem.edit', $data);
    }
    ...
    public function update(Request $r)
    {
        ...
        $result = $this->model->doEdit($data, 'restaurantitemmaster', $r->code);
		if ($r->hasFile('itemImage')) { 
				$path = 'uploads/restaurant/restaurantitemimage/'.$r->restaurantCode;
				if (! File::exists($path)) {
					File::makeDirectory($path,0777,true);
        ...
        if ($result) {
			...
			//activity log start
			$data=$currentdate->toDateTimeString().	"	"	.$ip.	"	"	.Auth::guard('admin')->user()->code.	"	"."Restaurant Item ".$r->code." is updated.";
			$this->model->activity_log($data); 
			//activity log end
            return redirect('restaurantItem/list')->with('success', 'Restaurant item updated successfully');
        }
    ...
    public function view($code)
    {
		...
		$data['tags'] = $this->model->selectActiveDataFromTable("tagmaster"); 
        return view('admin.restaurantitem.view', $data);
    }
    ...
    public function delete(Request $r)
    {
        ...
        //activity log start
		$data1=$currentdate->toDateTimeString().	"	"	.$ip.	"	"	.Auth::guard('admin')->user()->code.	"	"."Restaurant Item".$code." is deleted";
	...
	public function deleteAddonCategory(Request $r)
	{
		...
		$currentdate = Carbon::now();
		//activity log start
		$data=$currentdate->toDateTimeString().	"	"	.$ip.	"	"	.Auth::guard('admin')->user()->code.	"	"."Add on category".$code." is deleted";
		$this->model->activity_log($data); 
		//activity log end
	...
	public function deleteAddonLine(Request $r)
	{
		...
		$currentdate = Carbon::now();
		//activity log start
		$data=$currentdate->toDateTimeString().	"	"	.$ip.	"	"	.Auth::guard('admin')->user()->code.	"	"."Add on category line".$code." is deleted";
		$this->model->activity_log($data); 
		//activity log end
	...
	public function addAddonLine(Request $r){
		...
		if ($duplicate) {
			...
			$code = $this->model->addNew($data, 'customizedcategorylineentries', 'CCLN');
			if ($code != 'false') {
			//activity log start
			$data=$currentdate->toDateTimeString().	"	"	.$ip.	"	"	.Auth::guard('admin')->user()->code.	"	"."Customizedcategory line entry".$code." is added";
			$this->model->activity_log($data); 
			//activity log end
	...
	public function addAddonCategory(Request $r)
	{
		...
		if ($duplicate) {
			...
			$code = $this->model->addNew($data, 'customizedcategory', 'ITCC');
			if ($code != 'false') {
				//activity log start
				$data=$currentdate->toDateTimeString().	"	"	.$ip.	"	"	.Auth::guard('admin')->user()->code.	"	"."Customizedcategory".$code." is added";
				$this->model->activity_log($data); 
				//activity log end
				$response['status']  = 'true';
				$response['code']  = $code;
	...
	public function updateAddonCategory(Request $r)
	{
		...
		if ($duplicate) {
			...
			if ($resultUpdate) {
				//activity log start
				$data=$currentdate->toDateTimeString().	"	"	.$ip.	"	"	.Auth::guard('admin')->user()->code.	"	"."CustomizedCategory ".$customizedCategoryCode." is updated.";
				$this->model->activity_log($data); 
				//activity log end
				$response['status']  = 'true';
	...
	public function getSubcategoryDetails(Request $request)
	{
		...
		$subcategorydetails = $this->model->selectQuery(array('menusubcategory.code','menusubcategory.menuSubCategoryName'),'menusubcategory',array(),array('menusubcategory.isActive' => array('=', 1),'menusubcategory.menuCategoryCode' => array('=', $menuCategoryCode)));
		$html = "";
	...
	public function uploadExcel(){
		return view('admin.restaurantitem.upload');
	}
	public function uploadData(Request $r){
		...
		try {
			...
			foreach ($allDataInSheet as $value) {
				...
				$restaurantCode = $this->getCodeFromName('restaurant',$value['B'],'entityName');
				$inserdata[$i]['restaurantCode'] = $restaurantCode;
				$inserdata[$i]['salePrice'] = $value['C'];
				$inserdata[$i]['itemPackagingPrice'] = $value['D'];
				$inserdata[$i]['maxOrderQty'] = $value['E'];
				$inserdata[$i]['cuisineType'] = $value['F'];
				$inserdata[$i]['itemDescription'] = $value['H'];
				$menuCategoryCode = $this->getCodeFromName('menucategory',$value['I'],'menuCategoryName');
				$inserdata[$i]['menuCategoryCode'] = $menuCategoryCode;
				$menuSubcategoryCode = $this->getCodeFromName('menusubcategory',$value['J'],'menuSubCategoryName');
				$inserdata[$i]['menuSubCategoryCode'] = $menuSubcategoryCode;
				$itemActive=$adminApprove=0;
				if(strtolower($value['K'])=='yes') $itemActive=1;
				$inserdata[$i]['itemActiveStatus'] = $itemActive;
				if(strtolower($value['L'])=='yes') $adminApprove=1;
				$inserdata[$i]['isAdminApproved'] = $adminApprove;
				$inserdata[$i]['isActive'] = 1;
				$inserdata[$i]['isDelete'] = 0;
				$inserdata[$i]['addIP'] = $_SERVER['REMOTE_ADDR'];
				$inserdata[$i]['addID'] = Auth::guard('admin')->user()->code;
				...
				foreach($allData -> getDrawingCollection() as $drawing) {
					if($incr<count($itemCodeArr)){
						...
						if ($drawing instanceof \PhpOffice\PhpSpreadsheet\Worksheet\BaseDrawing) {
							...
							if($cellID==\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(7).$row){
	...
	public function generateExcelTemplate(){
		...
		$sheet->setCellValue('C1', 'Sale Price');
		$sheet->setCellValue('D1', 'Packaging Charges');
		$sheet->setCellValue('E1', 'Max Order Qty');
		$sheet->setCellValue('F1', 'Cuisine Type');
		$sheet->setCellValue('G1', 'Item Image');
		$sheet->setCellValue('H1', 'Item Description');
		$sheet->setCellValue('I1', 'Menu Category');
		$sheet->setCellValue('J1', 'Menu Subcategory');
		$sheet->setCellValue('K1', 'Item Status');
		$sheet->setCellValue('L1', 'Approved');
	...
	public function validateExcelFile(Request $r){
        ...
        $restaurantLineStr = 'Invalid Restaurant: ';
        ...
        $approvedLineStr = 'Invalid Approved Status: ';
        $str = $restaurantStr = $cuisineStr = $menuCateStr=$menuSubcStr=$itemStatusStr=$approvedStr='';
		$statusArr = ['yes','no'];
		$cuisineTypeArr = ['veg','nonveg'];
        $returnRowArray = [];
        for ($i = 0; $i < count($rowArray); $i++) {
            if (!in_array(strtolower($rowArray[$i][6]), $cuisineTypeArr)) {
                $cuisineStr = $cuisineStr . " Row" . ($i + 1) . ", ";
				 if (!in_array($i, $returnRowArray)) {
					array_push($returnRowArray, $i);
				 }
            }
			if (!in_array(strtolower($rowArray[$i][11]), $statusArr)) {
                $itemStatusStr = $itemStatusStr . " Row" . ($i + 1) . ", ";
				 if (!in_array($i, $returnRowArray)) {
					array_push($returnRowArray, $i);
				 }
            }
			if (!in_array(strtolower($rowArray[$i][12]), $statusArr)) {
                $approvedStr = $approvedStr . " Row" . ($i + 1) . ", ";
				 if (!in_array($i, $returnRowArray)) {
					array_push($returnRowArray, $i);
				 }
            }
            $restCode = $this->getCodeFromName('restaurant',$rowArray[$i][2],'entityName');
			if($restCode=='invalid'){
                $restaurantStr = $restaurantStr . " Row" . ($i + 1) . ", ";
                if (!in_array($i, $returnRowArray)) {
                    array_push($returnRowArray, $i);
                }
            }
			$menuCateCode = $this->getCodeFromName('menucategory',$rowArray[$i][9],'menuCategoryName');
			if($menuCateCode=='invalid'){
				...
				$extraCondition = " lower(menusubcategory.menuSubCategoryName)='".strtolower($rowArray[$i][10])."'";
        ...
        $msg = '';
        if ($restaurantStr != '') $msg = $msg . '<li><h5><b>'.$restaurantLineStr.' : </b><span style="color:#F68D0F">'. rtrim($restaurantStr, ', ').'</span></h5></li>';
		...
		if ($approvedStr != '') $msg = $msg . '<li><h5><b>'.$approvedLineStr.' : </b><span style="color:#F68D0F">'. rtrim($approvedStr, ', ').'</span></h5></li>';
