@extends('admin.layout.master', ['pageTitle'=>"Add Restaurant Item Addon"])
@push('styles')
						...
						@if($categories)
							@foreach($categories as $c)
								...
								@endif
								 ...
								 @endif
								<a class="nav-link" id="{{ $c->code}}-tab" data-toggle="pill" href="#{{ $c->code}}" role="tab" aria-controls="{{ $c->code}}" aria-selected="true"><div class="row"><div class="col-md-7">{{ $c->categoryTitle}} - {{ $categoryType}} - {{ $enabled}}</div><div class="col-md-5"> <span class="mr-1 ml-2 btn btn-warning btn-xs deleteCustomizedCategory" id="deltab_{{ $c->code}}"><i class="fa fa-trash"></i></span><span class="mr-1 btn btn-danger btn-xs editCustomizedCategory" id="edttab_{{ $c->code}}"><i class="ti-pencil-alt"></i></span></div></div></a>
							@endforeach
						@endif
						</div>
                    </div>
...
@push('scripts')
  ...
  <script type="text/javascript" src="{{ asset('assets/theme/dist/sweetalert/sweetalert2.min.js') }}"></script>
	 <script type="text/javascript" src="{{ asset('assets/init_site/admin/restaurantitem/addon.js') }}"></script>
