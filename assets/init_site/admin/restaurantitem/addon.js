...
$("body").on('click',".deleteCustomizedCategory",function(e){
	...
	$.ajax({
		url: baseUrl+'/restaurantItem/deleteAddonCategory',
		data:{
			'code':code
		},
		type:'get',
	...
	$("body").on('click',".lineDelete",function(e){
		...
		$.ajax({
			url:baseUrl+'/restaurantItem/deleteAddonLine',
			data:{'code':code},
			type:'get',
			success:function(response){   
				if(response==true){
		...
		if(subTitle !=''){
			if(price !=''){
				if(subTitle.length>2){
					$.ajax({
						url:baseUrl+'/restaurantItem/addAddonLine',
	...
	$("body").on('click',".editCustomizedCategory",function(e){
		...
		code = code.substring(7);
		$.ajax({
			url:baseUrl+'/restaurantItem/getAddonCategoryData',
			data:{'code':code},
			type:'get',
			success:function(response){  
				if(response != ""){
	...
	$("body").on('click',"#addCustomizedCategory",function(e){ 
		...
		var url = baseUrl+'/restaurantItem/addAddonCategory';
		if(customizedCategoryCode!="" && customizedCategoryCode!=undefined) url = baseUrl+'/restaurantItem/updateAddonCategory';
		var isCateEnabled  = 0;
